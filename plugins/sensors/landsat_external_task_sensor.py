    ...
    *not* fail if the external task fails, but will continue to check the status
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.log.info('skipped_states\t:{}'.format(skipped_states))
        self.skipped_states = list(skipped_states) if skipped_states else []
        self.log.info('failed_states\t:{}'.format(failed_states))
    ...
    def _handle_execution_date_fn(self, context) -> Any:
        ...
        # add external_dag_id and external_task_id to kwargs for backward compatibility
        kwargs["external_dag_id"] = self.external_dag_id
        kwargs["external_task_id"] = self.external_task_id
        if TYPE_CHECKING:
